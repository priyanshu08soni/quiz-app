  ...
  const handleQuizCreation = async (e) => {
    ...
    try {
      ...
      if (success) {
        ...
        toast.error(result.error?.details?.[0]?.message || message);
      }
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      <div className="mt-6">
        ...
        <button
          onClick={addQuestion}
          className="bg-primary text-white px-4 py-2 rounded-full mt-4 hover:bg-primary/90 transition"
        >
          Add Question
        </button>
      </div>
      ...
      <div className="mt-6">
        ...
        {questions.length > 0 ? (
          questions.map((q, index) => (
            <div key={index} className="border p-3 rounded mt-2 flex justify-between items-center">
              <div>
